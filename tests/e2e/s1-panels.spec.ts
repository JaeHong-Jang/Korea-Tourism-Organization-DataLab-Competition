...
test.beforeAll(async () => {
  server = spawn(process.execPath, [resolve(process.cwd(), "../../node_modules/vite/bin/vite.js"),
    "--host", "127.0.0.1", "--port", "5194", "--strictPort"],
  { cwd: process.cwd(), stdio: "ignore" });
  for (let attempt = 0; attempt < 60; attempt++) {
    if (server.exitCode !== null) break;
    try { if ((await fetch(origin)).ok) return; } catch { /* 기동 중인 서버를 기다린다. */ }
...
async function selection(page: import("@playwright/test").Page) {
  return page.evaluate(async () => {
    ...
    return { festival: state.selectedFestivalId, sigungu: state.selectedSigunguCode };
  });
}
...
test("필터·목록·KPI와 선택", async ({ page }) => {
  ...
  await expect(page.locator(".festival-filters__actions")).toContainText("적용 3개");
  await page.locator(".festival-list__pick").first().click();
  await expect(page.locator(".festival-list__items > li").first()).toHaveClass(/is-selected/);
  expect(await selection(page)).toEqual({ festival: "e-scene-13", sigungu: "11110" });
  await page.locator(".festival-list__pick").first().press("ArrowDown");
  expect(await selection(page)).toEqual({ festival: "e-scene-1", sigungu: "11110" });
  await page.getByRole("button", { name: "모두 초기화" }).click();
  await expect(page.locator(".festival-list__items > li")).toHaveCount(30);
  await expect(page.locator(".festival-filters__actions")).toContainText("적용 0개");
});
...
test("SVG 대체 지도에서 선택과 필터", async ({ page }) => {
  ...
  await expect(page.getByRole("img", { name: "행사와 시군구를 선택할 수 있는 SVG 전국 지도" })).toBeVisible();
  await expect(page.locator(".svg-korea-map__event")).toHaveCount(30);
  await page.getByRole("button", { name: "견본 행사 1, 1등급 선택" }).click({ force: true });
  expect(await selection(page)).toEqual({ festival: "e-scene-1", sigungu: "11110" });
  await expect(page.locator(".festival-list__items li.is-selected")).toHaveCount(1);
...
test("모바일 패널 화면", async ({ page }) => {
  ...
  await expect(page.locator(".festival-list__items > li")).toHaveCount(30);
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: resolve(screens, "T-404-s1-mobile.png"), fullPage: true });
});
