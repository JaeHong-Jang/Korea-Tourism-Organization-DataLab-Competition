...
- **작업 중인 레인 워크트리에는 develop을 병합하지 않는다**(워커가 끝나고 게이트를 마친 뒤에만 `worktree.sh`). 웹 워크트리는 `worktree.sh`가 `apps/web/public/geo/sigungu.topo.json`을 본 레포 경계 파일로 링크한다.
...
> **구현 우선 모드(9/25 오후 사용자 결정, 제출 전까지)**: task마다 게이트 1(허용 경로)·게이트 2(테스트·빌드·lint 재실행)만 통과하면 병합한다. 게이트 3(교차 리뷰)·게이트 4(실사용·스크린샷)와 R 검토는 **구현을 모두 끝낸 뒤 전체 QA 한 번**(통합 브랜치 전체 E2E·실사용·묶음 교차 리뷰)으로 몰아서 하고, 거기서 나온 문제만 고친다. 게이트 2의 작은 실패는 재디스패치 없이 오케스트레이터가 바로 고친다.
