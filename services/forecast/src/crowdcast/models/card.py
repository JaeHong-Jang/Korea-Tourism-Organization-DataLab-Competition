...
def model_card(
    ...
    scale_note = (
        f"규모 계층: 발표치 사용 {used}건(조건부 행사 속성 — 개최계획서 전년 방문객 신고치, 공개일 미입증). "
        "전회차 라벨 → 주어진 발표치 일평균 × 보정비율의 역수 → 유형 중앙값 순이다. "
        "파일명 민감도는 기존 속성 가림을 거친 발표치를 쓴다. 명시된 공개일이 기준일 이후이거나 "
        "0·결측·기간 미상·학습 보정 쌍 부재이면 유형으로 대체한다. "
