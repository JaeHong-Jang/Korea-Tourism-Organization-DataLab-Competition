...
def model_card(
    ...
    scale_note = (
        f"규모 계층: 발표치 사용 {used}건(조건부 단순 모델 평가 표본). "
        "전회차 라벨 → 공개일 ≤ D-14인 발표치 일평균 × 보정비율의 역수 → 유형 중앙값 순이다. "
        "발표 공개일 미상·기준일 이후·0·기간 미상·학습 보정 쌍 부재는 유형으로 대체한다. "
        "발표치 계층의 80% 폭은 학습 발표치↔라벨 로그 잔차의 가중 분위수로 추정한다. "
        "발표치 과장 가능성은 보정비율로 줄이려 했지만 행사장 실측은 아니며 보정 효과는 미보장이다. "
        if config.get("simple_announced_scale", False)
        else "발표치로 선택한 규모 계층이 비면 유형 중앙값을 유지하므로 작은 행사 규모 편향이 남는다. "
    )
    counts = (
    ...
    notes = (
        ...
        "지역 관측·전회차 라벨은 공개 시점 규칙을 적용한다. 입력 행사 일정·장소는 예보 대상 정의이고 "
        "환산은 같은 행사 정의에 적용한다. "
