...
def test_backtest_determinism_and_g0(
    ...
    with pytest.raises(FileNotFoundError):
        run_backtest(frame, NAMES, events, config, tmp_path / "g0.json", "a" * 64, tmp_path)
    path = freeze_g0(tmp_path, label_qc, "a" * 64, config["eval_years"], "v1-test")
    before, timestamp = path.read_bytes(), path.stat().st_mtime_ns
    first = run_backtest(frame, NAMES, events, config, path, "a" * 64, tmp_path)
    first_summary = summary(first, "bt-test", "v1-test")
    second = run_backtest(frame, NAMES, events, config, path, "a" * 64, tmp_path)
...
def test_evaluation_cannot_select_model(
    ...
    path = freeze_g0(tmp_path, label_qc, "a" * 64, [2025], "v1-test")
    before = path.read_bytes()
    original = run_backtest(frame, NAMES, events, config, path, "a" * 64, tmp_path)
    ...
    altered = run_backtest(changed, NAMES, events, config, path, "a" * 64, tmp_path)
...
def test_insufficient_training_skips_year(
    ...
    path = freeze_g0(tmp_path, label_qc, "a" * 64, config["eval_years"], "v1-test")
    result = run_backtest(frame, NAMES, events, config, path, "a" * 64, tmp_path)
...
def test_golden_replay(model_data: tuple, config: dict, label_qc: dict, tmp_path: Path) -> None:
    frame, events = model_data
    config["eval_years"] = [2025]
    ...
    path = freeze_g0(tmp_path, label_qc, "a" * 64, [2025], "v1-golden")
    result = run_backtest(ordinary, NAMES, events, config, path, "a" * 64, tmp_path, golden)
