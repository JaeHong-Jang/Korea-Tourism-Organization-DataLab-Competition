...
use CrowdCast\Records\Routes\RecordsRoutes;
use DI\Container;
use PDO;
...
// 상태 확인·기록 라우트와 공통 JSON 오류 응답을 조립한다
final class App
{
    // 테스트와 HTTP 진입점이 같은 라우트 및 오류 처리 구성을 사용한다(테스트는 메모리 DB를 넘긴다)
    /** @return SlimApp<Container> */
    public static function create(?PDO $db = null): SlimApp
    {
        ...
        // 행사·스냅샷 라우트는 첫 기록 요청 때 저장소를 연다
        RecordsRoutes::register($app, $db);
