...
- **실서비스 확인(9/25 저녁, 확인용 서버·실제 Ollama)** — 상담(수원) → 게이트 A·B → 발행 13문장(매뉴얼 규칙 6개 포함) → 계획 초안(인력 3·조직 2 '작성됨') → docx·작성자 메모 → what-if "일요일이면?"(새 예보 발행)·"비 오면?"(날씨 근거 없음 안내) → 저장 → 재예보 확인. 여기서 고친 것: 재예보가 평시·유사 근거를 적재하지 않아 knowledge 422(034eb08), 같은 날 같은 조건 재예보는 예보 id가 같아 503 → 409 변화 없음(뒤 커밋), 운영 상태 첫 호출 503(39fe35f), 날씨 없음 안내 원인 문구(33bfdd8), S5 이력 '조건 바꿈' 표시(c4db626).
- **영종(28110) 예보 불가**: 인천 개편 지역 2026-01~06 방문자 자료 필요 → 9/26 수집 뒤. **상담 예시 칩이 영종이라 그 전까지 예시 칩은 "관측 없음" 오류로 끝난다.**
- **records 중계 4xx**: records가 422(초안 발행 문장 수정 거부)를 줘도 게이트웨이는 503으로 바꾼다 — 편집기는 메모만 보내 실제로는 안 걸림. 4xx를 그대로 넘길지 G3 전에 결정.
- **본 레포 node_modules**가 2주차 의존성 전이라 본 레포에서 web test/build를 돌리면 모듈 없음 — 워크트리·리눅스 체크아웃은 최신. 본 레포에서 돌릴 때는 `npm ci` 먼저.
- **스크린샷 v2 검토(9/25 밤, `reports/figures/screens/v2/` 132장)** — 다음 다듬기(R-10 전):
  - `s1-3d-*-night`: 판과 **바다 위**에 작은 점이 흩뿌려짐(밤 조명·입자 추정) — 육지 타일 위로 제한.
  - `s5-reforecast-*`: 근거 칩이 원시 id(`ev-weather-…`)를 보임 → 근거 제목, "새 예보서 보기"를 링크 버튼으로.
  - S5 제목 "내행사" → 메뉴와 같게 "내 행사"(원칙 3).
- **실측 시나리오 평가(9/25 밤, `reports/evals/scenario-2026-09-25.*` — latest.json = live)**: 25/26, 발행 문장 211/211 근거·숫자 검산, 되묻기 6/6·후속 의도 6/6, 발행 지연 p50 3.85초·p95 4.35초(n 12), 모델 qwen3:4b. 실패 W03("밤이면?") — 바꾼 조건이 원래와 같아 예보 id가 같음 → SERVICE_UNAVAILABLE. **what-if도 재예보처럼 "조건이 같아 예보가 같아요" 안내로**(T-447 뒤 게이트웨이).
