...
// /facts로 허용되는 전이(이전 → 다음). 발행(candidate → published)은 /publish만 한다. candidate → candidate는 재검사(recheckedLater)일 때만
...
// 새 검사 결과가 이전보다 모두 더 새 revision에서 나왔는지(검사 종류는 그대로 다 있어야 한다)
function recheckedLater(prevChecks, nextChecks) {
  const prevMax = Math.max(...prevChecks.map((c) => c.revision));
  const kinds = (xs) => canonical([...new Set(xs.map((c) => c.checkKind))].sort());
  return nextChecks.length > 0 && kinds(prevChecks) === kinds(nextChecks) && nextChecks.every((c) => c.revision > prevMax);
}
...
export function factsTransitionProblems(prev, next) {
  ...
  if (prev.status === next.status) {
    // 재검사: candidate는 본문(rendered 포함)을 그대로 두고 더 새 revision의 검사 결과로만 바꿀 수 있다(새 사실이 들어온 뒤)
    const recheck = prev.status === "candidate" && canonical(prev.rendered) === canonical(next.rendered) && recheckedLater(prev.checks, next.checks);
    if (!recheck) out.push(`claim ${next.id}: 상태 ${prev.status} 그대로는 더 새 revision의 검사 결과로만 바꿀 수 있다(본문은 그대로)`);
  }
