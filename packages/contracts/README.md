# packages/contracts — 서비스 간 계약 v1.2 (오케스트레이터 소유, 워커는 읽기 전용)
...
| `rules/integrity.mjs` | 참조 무결성 판정(`refProblems`) — 근거 그래프 적재(T-601)가 Python으로 같은 판정을 구현 |
...
| `fixtures-sse/` | SSE 이벤트 순서 정상·위반 예시 |
